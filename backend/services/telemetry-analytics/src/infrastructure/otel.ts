...
// (server.ts) before Hono, or pino imports, so that the global OTel tracer
// provider is registered before instrumentation libraries patch built-ins.
//
...
const serviceName = process.env.SERVICE_NAME ?? 'telemetry-analytics';
